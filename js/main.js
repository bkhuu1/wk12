...
document.addEventListener('DOMContentLoaded', function () {
    function enableWrapAround(retries = 10) {
        if (window.lightbox && typeof lightbox.option === 'function') {
            lightbox.option({ wrapAround: true });
        } else if (retries > 0) {
            setTimeout(() => enableWrapAround(retries - 1), 200);
        } else {
            console.warn('Could not enable Lightbox wrapAround. Consider loading dist/js/lightbox-plus-jquery.js before js/main.js.');
        }
    }
    enableWrapAround();
});